...
export const createType = async () => {//вставить что передаем
    const {data} = await $authHost.post('api/type')//вставить айпишник , поменять order на то что перердаем
    return data
}
...
export const createOrder = async () => { //вставить что передаем
    const {data} = await $authHost.post('api/device') //вставить айпишник 
    return data
}
// Аналогично
